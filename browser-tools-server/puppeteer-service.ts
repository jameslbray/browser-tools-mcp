...
const DEFAULT_CONFIG: PuppeteerServiceConfig = {
	preferredBrowsers: ['chrome', 'chromium', 'edge', 'brave', 'firefox'],
 ...
 */
export function configurePuppeteerService(config: Partial<PuppeteerServiceConfig>): void {
	currentConfig = { ...DEFAULT_CONFIG, ...config };

	// Update the timeout if it was changed
	if (config.browserCleanupTimeout && config.browserCleanupTimeout !== BROWSER_CLEANUP_TIMEOUT) {
	...
	if (
		...
		for (const browser of preferredBrowsers) {
			if (
				...
				launchOptions.executablePath = currentConfig.customBrowserPaths[browser];
...
async function findBrowserExecutablePath(): Promise<string> {
	...
	if (platform === 'win32') {
		...
		// Check each browser in preferred order
		for (const browser of preferredBrowsers) {
			const paths = macBrowserPaths[browser as keyof typeof macBrowserPaths] || [];
			for (const browserPath of paths) {
				if (fs.existsSync(browserPath)) {
					console.log(`Found ${browser} at ${browserPath}`);
 ...
 */
function setupBrowserCleanupHandlers(browser: puppeteer.Browser, userDataDir: string): void {
